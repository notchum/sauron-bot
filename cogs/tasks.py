...
class Tasks(commands.Cog):
    ...
    async def clean_temp_dir(self):
        ...
        if self.clean_temp_dir.current_loop == 0:
            return
        self.bot.logger.debug(f"Cleaning temp directory... [loop #{self.clean_temp_dir.current_loop}]")
        self.bot.clean_temp_dir()
        self.bot.logger.info("Finished clearing temp directory.")

    @tasks.loop(hours=1.0)
    async def check_for_media(self):
        self.bot.logger.debug(f"Checking for absent media... [loop #{self.check_for_media.current_loop}]")

        for channel_id in self.bot.monitored_channels:
            channel = await self.bot.fetch_channel(channel_id)
            self.bot.logger.info(f"Searching channel {channel.name}[{channel.id}] for media...")
